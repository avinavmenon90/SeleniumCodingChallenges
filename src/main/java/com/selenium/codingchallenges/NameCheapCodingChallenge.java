...
/*
 * 1) go to this url - https://www.namecheap.com/
2) Search for a particular website and check whether the website is already taken or not
 */


public class NameCheapCodingChallenge {
	
	static WebDriver driver;
	static WebDriverWait expWait;
	static int counter;
	static WebElement searchBtn,searchBox;
	...
	public static void main(String[] args) throws InterruptedException {
		...
		/*
		 * //clicking on a random section (to dismiss Login banner) action = new
		 * Actions(driver); action.moveToElement(driver.findElement(By.
		 * xpath("//div[@id=\"root\"]//h1[text()='Search Flights']")))
		 * .doubleClick().build().perform();
		 */
		 
	//Search for a website domain
		 searchBox = driver.findElement(By.id("static-domain-search-domain-search-input"));
		 searchBox.sendKeys("sdet.to");
		 
	//click on Search
		 searchBtn = driver.findElement(By.xpath("//*[@id=\"static-domain-search\"]//button[@type='submit'][@aria-label='Search']"));
		 searchBtn.click();
		
		//section[@class='standard']//span[@class='label taken'][contains(text(),'Taken')]
